            ...
            <div id="kt_app_toolbar_container" class="app-container container-fluid d-flex flex-stack">
                <div class="page-title d-flex flex-column justify-content-center flex-wrap me-3">
                    <ul class="breadcrumb breadcrumb-separatorless fw-semibold fs-7 my-0 pt-1">
                        ...
                        <li class="breadcrumb-item text-muted">BOT</li>
                    </ul>
                </div>
            </div>
        </div>
                    ...
                    <div class="alert alert-danger d-flex align-items-center p-5 mb-5">
                        <span class="svg-icon svg-icon-2hx svg-icon-danger me-4">
                            <svg width="24" height="24" viewBox="0 0 24 24" fill="none"
                                xmlns="http://www.w3.org/2000/svg">15
                                <path opacity="0.3"
...
<script>
  $(document).ready(function() {
        $('#website').change(function() {
            ...
            var storeUrl = selectedOption.data('store_url');
            var consumerKey = selectedOption.data('consumer_key');
            var consumerSecret = selectedOption.data('consumer_secret');
            ...
            $.ajax({
                ...
                success: function(data) {
                    var webCategorySelect = $('#webCategory');
                    webCategorySelect.empty();

                    $.each(data, function(index, category) {
                        var option = $('<option></option>');
                        option.val(category.id);
                        option.text(category.name);
                        option.addClass('mb-2 fs-6 fw-bold');
                        webCategorySelect.append(option);
                    });
                },
