...
@section('title','Kuyruktaki İşler')
@section('content')
        ...
        <div id="kt_app_toolbar_container" class="app-container w-100 d-flex flex-stack">
            <div class="page-title d-flex flex-column justify-content-center flex-wrap me-3">
                <ul class="breadcrumb breadcrumb-separatorless fw-semibold fs-7 my-0 pt-1">
                    ...
                    <li class="breadcrumb-item text-muted">Kuyruktaki İşler</li>
                </ul>
            </div>
             <div class="d-flex align-items-center gap-2 gap-lg-3">
                 <a href="{{route('job.fail')}}" class="btn btn-sm fw-bold btn-danger"><i class="las la-bug fs-1"></i> Hatalı İşler</a>
             </div>
        </div>
    </div>
                     ...
                     <table class="table table-striped border rounded table-row-dashed fs-6  px-0 mx-0 overflow-x-scroll" id="kt_datatable_example">
                         <thead class="py-12 px-2">
                             <tr class="text-gray-700 fw-bold text-uppercase bg-light w-100 px-0 mx-0">
                                 <th class="text-start px-12">ID</th>
                                 <th class="text-start px-12">Queue</th>
                                 <th class="text-start px-12">Url</th>
                                 <th class="text-start px-12">Reservet At</th>
                                 <th class="text-start px-12">Created At</th>
                                 <th class="text-end px-12">İşlemler</th>
                             </tr>
                         </thead>
                         <tbody class="fw-semibold text-gray-600">
                             @foreach ($jobs as $job)
                             <tr class="{{$job->reserved_at ? 'bg-success' : ''}}">
                                ...
                                <td class="text-start px-md-10 px-12 py-6">
                                    {{$job->reserved_at ? \Carbon\Carbon::createFromTimestamp($job->reserved_at)->toDateTimeString() : 'N/A'}}
                                </td>
                                <td class="text-start px-md-10 px-12 py-6">
                                 ...
                                 <td class="text-end px-md-12 px-12 py-4">
                                     ...
                                     <a onclick="deleteJob('{{route('job.delete',$job->id)}}')" class="btn btn-icon btn-bg-light btn-active-color-primary btn-sm bg-gray-200i">
